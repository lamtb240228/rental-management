...
> **Trạng thái tài liệu:** đây là thiết kế đích/lịch sử ban đầu, không phải mô tả vật lý chính xác của database đang chạy. Flyway trong `backend/src/main/resources/db/migration` (hiện V1–V8) là nguồn sự thật. Các cột/trạng thái chỉ xuất hiện ở tài liệu này phải được xem là backlog cho đến khi có migration mới tương ứng; không sửa migration đã chạy để ép schema khớp tài liệu.
...
| `auth_version`   | `BIGINT`        | No   | Default `0`, Check `>= 0`          | Phiên bản vô hiệu access token  |
| `created_at`     | `TIMESTAMPTZ`   | No   | Default current time               | Thời gian tạo                   |
| `updated_at`     | `TIMESTAMPTZ`   | No   | Default current time               | Thời gian cập nhật              |
...
* `auth_version` không âm và chỉ tăng khi credential hoặc trạng thái bảo mật yêu cầu revoke-all; V8 backfill account cũ bằng `0`.
...
### 2.4. refresh_sessions

Migration [V7__create_refresh_sessions.sql](../backend/src/main/resources/db/migration/V7__create_refresh_sessions.sql) tạo bảng `refresh_sessions` để quản lý refresh token có rotation/revocation. Raw refresh token không bao giờ được lưu trong database; `token_hash` là SHA-256 hash dạng hex của opaque token 256 bit.

| Column | PostgreSQL Type | Null | Constraint | Description |
| --- | --- | --- | --- | --- |
| `id` | `BIGINT` | No | Primary Key, Identity | Mã của từng token generation |
| `user_account_id` | `BIGINT` | No | Foreign Key, `ON DELETE RESTRICT` | Account sở hữu refresh session |
| `token_hash` | `VARCHAR(64)` | No | Unique | SHA-256 hash; không chứa raw token |
| `family_id` | `UUID` | No | | Logical session family được giữ nguyên khi rotate |
| `created_at` | `TIMESTAMPTZ` | No | Default current time | Thời điểm token generation được tạo |
| `expires_at` | `TIMESTAMPTZ` | No | Check sau `created_at` | Absolute expiry của family |
| `last_used_at` | `TIMESTAMPTZ` | Yes | Không trước `created_at` | Thời điểm generation được dùng để rotate |
| `revoked_at` | `TIMESTAMPTZ` | Yes | Không trước `created_at` | Thời điểm generation bị vô hiệu hóa |
| `replaced_by_session_id` | `BIGINT` | Yes | Self Foreign Key, `ON DELETE SET NULL` | Successor generation sau rotation |

Constraints vật lý của V7:

```text
uk_refresh_sessions_token_hash
ck_refresh_sessions_expiration
ck_refresh_sessions_last_used
ck_refresh_sessions_revoked
ck_refresh_sessions_not_self_replaced
```

Các quy tắc vòng đời:

* Login/register tạo `family_id` mới và một token generation đầu tiên.
* Rotation khóa row hiện tại, revoke generation cũ và liên kết `replaced_by_session_id` tới successor trong cùng transaction.
* Mọi generation trong family giữ cùng absolute `expires_at`; refresh không kéo dài session quá thời hạn mặc định 7 ngày.
* Record đã rotate được giữ đến khi hết hạn để phát hiện token reuse. Khi predecessor bị dùng lại, toàn bộ active generation của family bị revoke.
* Đổi mật khẩu, khóa/vô hiệu hóa account hoặc logout-all revoke mọi active refresh session của account.

Chi tiết token/cookie/error policy nằm trong [Authentication and session security](authentication-security.md). Rotation và replay handling tham chiếu [RFC 9700](https://www.rfc-editor.org/rfc/rfc9700.html) và việc không lưu credential trong browser storage theo [OWASP Session Management](https://cheatsheetseries.owasp.org/cheatsheets/Session_Management_Cheat_Sheet.html).

---

## 3. Authentication Relationships

Quan hệ giữa bốn bảng authentication:
...
user_accounts
      ...
      +---------------- N refresh_sessions
                              |
                              +---- optional successor trong cùng family
```
...
* Một tài khoản có thể có nhiều refresh session family và token generation.
* Một refresh generation có thể trỏ tới successor sau rotation; service bảo đảm predecessor và successor thuộc cùng family.

Bốn bảng authentication hiện có là:

1. `roles`
2. `user_accounts`
3. `user_roles`
4. `refresh_sessions` (được bổ sung bằng V7 sau schema MVP ban đầu)
...
Hệ thống Rental Management System hiện gồm 14 bảng vật lý sau migration V7.
...
erDiagram
    ...
    REFRESH_SESSIONS o|--o{ REFRESH_SESSIONS : replaced_by
...
| `user_accounts`    | `refresh_sessions`     | One-to-Many          | `refresh_sessions.user_account_id` |
| `refresh_sessions` | `refresh_sessions`     | Optional Self-Reference | `refresh_sessions.replaced_by_session_id` |
...
Do đó bảng này được tạo cuối cùng trong schema MVP ban đầu; V7 bổ sung `refresh_sessions` sau đó.

#### Step 14: refresh_sessions (V7)

`refresh_sessions.user_account_id` tham chiếu đến `user_accounts.id`; `replaced_by_session_id` là self-reference tùy chọn. Bảng được bổ sung sau schema MVP bằng V7 để không sửa các migration V1–V6 đã chạy.
...
refresh_sessions.user_account_id
refresh_sessions.family_id
refresh_sessions.expires_at WHERE revoked_at IS NULL
refresh_sessions(user_account_id, expires_at) WHERE revoked_at IS NULL
```
...
V7 tạo chính xác các index/unique index sau cho vòng đời refresh session:

| Tên | Cột/điều kiện | Mục đích |
| --- | --- | --- |
| `uk_refresh_sessions_token_hash` | Unique `token_hash` | Lookup token hash và ngăn trùng generation |
| `idx_refresh_sessions_user_account_id` | `user_account_id` | Revoke/list session theo account |
| `idx_refresh_sessions_family_id` | `family_id` | Revoke toàn family khi logout hoặc phát hiện reuse |
| `idx_refresh_sessions_active_expiration` | `expires_at WHERE revoked_at IS NULL` | Tìm/cleanup generation active đã hết hạn |
| `idx_refresh_sessions_user_active` | `(user_account_id, expires_at) WHERE revoked_at IS NULL` | Revoke hoặc kiểm tra active session của account |
...
* [ ] Raw refresh token không được lưu trong database; `token_hash` là unique.
* [ ] Refresh generation có absolute expiry, revocation time và successor link hợp lệ.
* [ ] Rotation/reuse được serialize và revoke đúng session family.
...
Database hiện có 14 bảng vật lý sau khi V7 bổ sung refresh session vào 13 bảng MVP ban đầu:
...
Spring Boot, PostgreSQL và Flyway V1–V8 hiện đã được triển khai; `refresh_sessions` là schema P0-03 đang hoạt động và V8 bổ sung `user_accounts.auth_version` để revoke access token tức thời ở cấp account. Các thay đổi tiếp theo vẫn phải đối chiếu [backlog P0–P3](product-backlog.md), chỉ bổ sung bằng migration version mới, rồi cập nhật tài liệu sau khi build/test/restore drill thực tế đạt.
