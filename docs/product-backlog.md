...
- **P0-03:** đã hoàn thành mô hình access JWT 15 phút chỉ giữ trong memory và opaque refresh session 7 ngày; V7 chỉ lưu token hash, V8 thêm `auth_version` để invalidation toàn account có hiệu lực ngay. Authentication dùng lock order user→refresh-session, login/password/status được kiểm tra lại dưới user lock, reuse/concurrent loser revoke đúng family và historical token không thể dùng logout-all để revoke family độc lập. Các POST refresh/logout/logout-all kiểm tra chính xác `Origin` hoặc fallback `Referer` theo cùng CORS allowlist trên decoded MVC path và fail-closed trước mọi session mutation. Frontend có offline logout tombstone/retry gắn generation, safe application error, serialized cookie-auth request khi Web Locks khả dụng, session recovery, single-flight/retry một lần và cross-tab signal không credential. Xem [tài liệu bảo mật xác thực](authentication-security.md).
...
- **Acceptance criteria:**
  ...
  - Access JWT bắt buộc mang `authVersion` khớp database; change-password, logout-all và account security transition tăng version, còn unlock/profile edit không làm token cũ sống lại.
  ...
  - Refresh/logout/logout-all chỉ chấp nhận `Origin` khớp chính xác CORS allowlist hoặc `Referer` cùng origin khi thiếu `Origin`; sibling/cross-site và request thiếu cả hai header bị từ chối 403 trước rotation/revocation. Non-browser client phải gửi allowed `Origin`.
  ...
  - Cross-tab chỉ truyền tín hiệu login/logout và generation không phải credential; generation khiến tín hiệu cũ không thể ghi đè login mới. Logout offline giữ tombstone không nhạy cảm, chặn bootstrap refresh và retry khi startup/online; explicit login được serialize sau logout local đang chạy.
...
- **Bảng database:** `user_accounts` (`auth_version` từ V8), `roles`, `user_roles`, `refresh_sessions` (V7); xem [physical schema](database-schema.md#24-refresh_sessions).
...
- **Validation và lỗi:** Chuẩn hóa email và password policy; generic JSON 401 cho credential/refresh không hợp lệ; validate TTL, token size, cookie name/path/SameSite; không ghi token vào URL, log, exception hoặc `toString()`; frontend không phát tán raw Axios request config ra ngoài HTTP layer.
- **Test:** Backend kiểm tra cookie/hash-at-rest, rotation/reuse/revocation, expiry/forgery, logout/logout-all, password/account revocation, production cookie, concurrent refresh và Origin/Referer rejection không mutation trên PostgreSQL Testcontainers/Flyway V1–V8. Frontend kiểm tra memory-only/legacy cleanup, reload recovery, single-flight 401, retry guard, refresh failure, Web Locks, cross-tab signal và late-response race. Cổng đóng item gồm backend clean test; frontend lint/typecheck/unit/build; E2E auth phù hợp; `npm audit`; Compose config; diff/secret scan.
- **Hardening/vận hành còn mở:** chưa có scheduled cleanup cho refresh-session hết hạn, rate limit riêng cho refresh, secret scanning tự động bằng Gitleaks trong CI, E2E chuyên biệt khi Web Locks bị vô hiệu hóa, và code splitting để giảm cảnh báo bundle frontend. Các mục này chưa được triển khai bởi thay đổi Origin/Referer hiện tại; bundle optimization thuộc P2, các mục còn lại phải được theo dõi trong hardening vận hành/CI.
