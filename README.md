...
- Migration V8 thêm `user_accounts.auth_version`. Claim cùng tên trong access JWT được so với database ở mỗi request; đổi mật khẩu, logout-all, khóa/vô hiệu hóa hoặc xóa mềm account làm token đã cấp mất hiệu lực ngay.
...
- Frontend khôi phục phiên qua cookie sau khi reload, gộp các 401 đồng thời vào một refresh promise và chỉ retry mỗi request một lần. Logout lỗi mạng giữ tombstone/session-generation không nhạy cảm để chặn tự refresh lại, bỏ qua tín hiệu tab cũ và retry khi ứng dụng khởi động hoặc có kết nối; login/register/refresh/logout được serialize bằng cùng Web Lock khi trình duyệt hỗ trợ để response cũ không xóa cookie của login mới.
...
Cổng kiểm tra P0-03 phải bao phủ migration mới V1–V8 và nâng cấp V7→V8 trên PostgreSQL Testcontainers; login/cookie/hash-at-rest; lock order user→refresh-session; refresh rotation, expiry, forgery, reuse và các race concurrent; logout/logout-all/change-password/account lock cùng invalidation bằng `auth_version`; frontend memory-only storage, bootstrap refresh, offline logout retry, safe application error, single-flight 401, retry tối đa một lần, cross-tab logout và late-response race. Chỉ đóng checkpoint khi backend clean test, frontend lint/typecheck/unit/build, E2E authentication phù hợp, `npm audit`, Docker Compose config, `git diff --check` và secret scan đều đạt.
