...
describe("sessionChannel", () => {
  it("broadcasts login/logout metadata without any credential or user data", () => {
    Object.defineProperty(globalThis, "BroadcastChannel", {
      configurable: true,
    ...
    expect(transport.posted[0]).toMatchObject({ kind: "login" });
    expect(Object.keys(transport.posted[0] as object).sort()).toEqual([
      "id",
      "issuedAt",
      "kind",
      "source",
    ]);
    const serialized = JSON.stringify(transport.posted[0]).toLowerCase();
  ...
  it("uses a transient token-free storage signal when BroadcastChannel is unavailable", () => {
    ...
    expect(JSON.parse(serialized)).toMatchObject({ kind: "logout" });
